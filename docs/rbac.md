...
Kubernetes RBAC 只开放资源只读、`pods/log` 读取、`pods` 删除，以及 `deployments/statefulsets/daemonsets/jobs/cronjobs/services/ingresses` 更新。资源只读覆盖 core/apps/batch/discovery/networking/storage 中 P0 资源：Namespace、Node、Pod、Endpoints、EndpointSlice、Event、ConfigMap、Service、PVC/PV、Deployment、StatefulSet、ReplicaSet、DaemonSet、Job、CronJob、Ingress、StorageClass。EndpointSlice 仅开放 `get/list/watch`，供 Service 和 Ingress 网络关联详情复用；Ingress 详情不需要额外 Kubernetes 权限。
