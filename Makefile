...
.PHONY: deps run-server migrate lint test test-integration build image deploy backup restore
...
test:
	go test $(GO_PACKAGES)
	go test -tags=integration -run '^$$' $(INTEGRATION_PACKAGES)
	cd web && npm run test

test-integration:
	./test/integration/resources_k3d.sh
...
image:
	docker build --build-arg GOPROXY="$(GOPROXY)" -f deploy/docker/ops-server.Dockerfile -t $(SERVER_IMAGE) .
