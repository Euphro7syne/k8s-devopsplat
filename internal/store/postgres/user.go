...
ON CONFLICT (user_id, role_id) DO NOTHING`, userID, roleName)
	...
	if err == nil && affected == 0 {
		var exists int
		if scanErr := s.db.QueryRowContext(ctx, "SELECT COUNT(1) FROM roles WHERE name = $1", roleName).Scan(&exists); scanErr != nil {
...
func (s *Store) ReplaceUserRoles(ctx context.Context, userID int64, roleNames []string) error {
	...
	var userExists int
	if err := tx.QueryRowContext(ctx, "SELECT COUNT(1) FROM users WHERE id = $1", userID).Scan(&userExists); err != nil {
